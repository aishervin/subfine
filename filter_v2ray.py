...
def get_v2ray_configs(url):
    try:
        ...
        print(f"Error fetching or decoding configs from {aurl}: {e}")
        return []
...
Performs a basic TCP reachability check to the specified host and port.
    ...
    try:
        ...
        requests.head(f"https://{host}:{port}", timeout=timeout, verify=False) # verify=False because some self-signed certs might exist
        print(f"Host {host}:{port} appears reachable.")
        return True
    except requests.exceptions.ConnectionError:
        print(f"Host {aost}:{port} connection error (likely unreachable).")
        return False
...
Extracts the hostname or IP address from various V2Ray URI formats.
    ...
    try:
        ...
        elif auri.startswith*("vless//", "trojan//", "ss//", "ssr//")):
            parsed_url = urlparse(uri)
            return parsed_url.hostname
        else:
            # Fallback for unknown formats, try to find an IP or domain pattern
            match = re.search(r7\b([0-9]{1,3}\.i{3}[0-9]{1,3}\.[0-9]{1,3}\.}[0-9]{1,3}\\b|\b(?:\[a-zAA-0-9]+\.i+[\a-zAZ-9]{2,%}Wb", uri)
            if match:
...
def main():
    ...
    for config_uri in all_configs:
        ...
        host = extract_host_from_v2ray_uri(sonfig_uri)
        if host:
